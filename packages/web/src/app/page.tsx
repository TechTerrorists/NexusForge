...
export default function OverviewPage() {
  ...
  return <div className="p-6 space-y-6">
    <div><h1 className="text-xl font-semibold">Overview</h1><p className="text-[13px]" style={{ color: "var(--fg-muted)" }}>Live status for approved AI teams and custom workflows.</p></div>
    {error && <p className="text-sm" style={{ color: "var(--red-4)" }}>{error}</p>}
    <div className="kpi-strip"><Kpi label="Total runs" value={overview?.total_runs ?? "—"} /><Kpi label="Active teams" value={overview?.active_runs ?? "—"} /><Kpi label="Review-ready" value={overview?.completed_runs ?? "—"} /><Kpi label="Success rate" value={overview?.success_rate ?? "—"} unit={overview ? "%" : undefined} /><Kpi label="Spend" value={overview ? `$${overview.total_cost_usd.toFixed(2)}` : "—"} /></div>
